...
{
  mpf_ptr OUTOUTOUT; /*out*/
  value _vres;

  struct camlidl_ctx_struct _ctxs = { CAMLIDL_TRANSIENT, NULL };
  camlidl_ctx _ctx = &_ctxs;
  mpf_init(OUTOUTOUT);
  _vres = camlidl_c2ml_mpf_mpf_ptr(&OUTOUTOUT, _ctx);
  camlidl_free(_ctx);
  return _vres;
}
	...
	value _v_ROP2)
{
  ...
  struct camlidl_ctx_struct _ctxs = { CAMLIDL_TRANSIENT, NULL };
  camlidl_ctx _ctx = &_ctxs;
  camlidl_ml2c_mpf_mpf_ptrm(_v_ROP1, &ROP1, _ctx);
  camlidl_ml2c_mpf_mpf_ptrm(_v_ROP2, &ROP2, _ctx);
  mpf_swap(ROP1, ROP2);
  camlidl_free(_ctx);
  return Val_unit;
}
